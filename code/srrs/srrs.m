...
% the statematrix is empty for the first day, so the innitial position will be set to 0.
  if isempty(stateMatrix)                           
     stateMatrix.volume=0;
  end

% open a position with 80% amount of the account balance when beta is larger than l2
  if beta>l2
